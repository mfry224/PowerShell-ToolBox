				...
				1 {
					forEach ($i in KTFCU_fnc_hostFind)
					{
						...
						if ($strName.length -gt 0) {
							...
							$folders = Get-ChildItem -Path "\\$i\C$\Users\" -Exclude admin*,public*,default*,all*,desktop*,$finalUser;
							...
							foreach ($folder in $folders){
								...
								Rename-Item -Verbose -Path $folder.FullName -NewName "$name";

								if ($folder.PSIsContainer){
									$parts = $folder.FullName.Split("\")
									$folderPath = $parts[0];
								...
								Remove-Item $folder -Force -Verbose -Recurse -Exclude admin*,public*,default*,all*,desktop*,$localName,$domainName;
								write-host "`n";
								$name++;
							};
						} else {
							...
							$name = Get-Random -minimum 1 -maximum 9999;
							$folders = Get-ChildItem -Path "\\$i\C$\Users\" -Exclude admin*,public*,default*,all*,desktop*;
							
							<# If the folder path\length is too long let's rename it to then be deleted without error #>
							foreach ($folder in $folders){
								write-host "PS-Suite: No logged on users detected! User folder $folder will be renamed for deletion`n";
								...
								Remove-Item $folder -Force -Verbose -Recurse -Exclude admin*,public*,default*,all*,desktop*;
								write-host "`n";
								$name++;
							};
						};
